    ...
    public static class DiagnosticsHook
    {
            ...
            pipelines.BeforeRequest.AddItemToStartOfPipeline(
                new PipelineItem<Func<WebworkContext, Response>>(
                    ...
                    ctx =>
                    {
                        ...
                        var resourcePrefix = string.Concat(diagnosticsConfiguration.Path, "/Resources/");

                        if (ctx.Request.Path.StartsWith(resourcePrefix, StringComparison.OrdinalIgnoreCase))
                        {
                            var resourceNamespace = "ShanDian.Webwork.Diagnostics.Resources";
                            ...
                            return new EmbeddedFileResponse(typeof(DiagnosticsHook).Assembly, resourceNamespace, Path.GetFileName(ctx.Request.Url.Path));
                        }

                        RewriteDiagnosticsUrl(diagnosticsConfiguration, ctx);

                        return diagnosticsConfiguration.Valid ? ExecuteDiagnostics(ctx, diagnosticsRouteResolver, diagnosticsConfiguration, serializer)
        ...
        private static Response ExecuteDiagnostics(WebworkContext ctx, IRouteResolver routeResolver, DiagnosticsConfiguration diagnosticsConfiguration, DefaultObjectSerializer serializer)
        {
            ...
            if (session == null)
            {
                ...
                view.WithCookie(new WebworkCookie(diagnosticsConfiguration.CookieName, String.Empty, true) { Expires = DateTime.Now.AddDays(-1) });
