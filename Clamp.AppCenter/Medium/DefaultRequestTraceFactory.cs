namespace Clamp.AppCenter.Medium
{
    ...
    public class DefaultRequestTraceFactory : IRequestTraceFactory
    {
        ...
        public IRequestTrace Create(Request request)
        {
            ...
            var comparer = (StaticConfiguration.CaseSensitive) ? StringComparer.Ordinal : StringComparer.OrdinalIgnoreCase;

            requestTrace.Items = new Dictionary<string, object>(comparer);

            requestTrace.RequestData = request;

            requestTrace.TraceLog = (StaticConfiguration.DisableErrorTraces) ? (ITraceLog)new NullLog() : (ITraceLog)new DefaultTraceLog();
