...
namespace ShanDian.AddIns.Print.Html.Compiler
{
    internal class DeferredSectionVisitor : HandlebarsExpressionVisitor
    {
        ...
        private DeferredSectionVisitor(CompilationContext context) : base(context)
        {
        }
...
#endif
            Expression path = HtmlTemplateExpression.Path(dsex.Path.Path.Substring(1));
        ...
        private Expression[] GetDeferredSectionTemplates(DeferredSectionExpression dsex)
        {
            ...
            switch (sectionPrefix)
            {
                ...
                default:
                    throw new HtmlTemplateCompilerException("Tried to compile a section expression that did not begin with # or ^");
            }
        }
        ...
        private static void RenderSection(object value, BindingContext context, Action<TextWriter, object> body, Action<TextWriter, object> inversion)
        {
            ...
            else if (HtmlTemplateUtils.IsFalsyOrEmpty(value))
            {
                inversion(context.TextWriter, context);
            }
