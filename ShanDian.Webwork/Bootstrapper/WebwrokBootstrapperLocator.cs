namespace ShanDian.Webwork.Bootstrapper
{
    ...
    /// </summary>
    public static class WebwrokBootstrapperLocator
    {
        private static IWebworkBootstrapper instance;

        /// <summary>
        /// Gets the located bootstrapper
        /// </summary>
        public static IWebworkBootstrapper Bootstrapper
        {
            get { return instance ?? (instance = LocateBootstrapper()); }
            set { instance = value; }
        }

        private static IWebworkBootstrapper LocateBootstrapper()
        {
            var bootstrapperType = GetBootstrapperType();

            try
            {
                return Activator.CreateInstance(bootstrapperType) as IWebworkBootstrapper;
            }
            catch (Exception ex)
            {
        ...
        private static Type GetBootstrapperType()
        {
            var customBootstrappers = AppDomainAssemblyTypeScanner
                .TypesOf<IWebworkBootstrapper>(ScanMode.ExcludeWebwork)
                .ToList();

            if (!customBootstrappers.Any())
            {
                return typeof(DefaultWebworkBootstrapper);
            }
