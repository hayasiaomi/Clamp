        ...
        CefReturnValue IRequestHandler.OnBeforeResourceLoad(IWebBrowser browserControl, IBrowser browser, IFrame frame, IRequest request, IRequestCallback callback)
        {
            if (!callback.IsDisposed)
            {
                using (callback)
                {
                    ...
                    DebugHelper.WriteLine(request.Method + ":" + request.Url);
                    ...
                    if (Uri.TryCreate(request.Url, UriKind.Absolute, out callUri))
                    {
                        if (callUri.DnsSafeHost.EndsWith("SD-proxy.chidaoni.com", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            request.Url = $"{callUri.Scheme}://{SDShell.SDShellSettings.ShanDianHost}:{SDShell.SDShellSettings.ShanDianPort}{ callUri.PathAndQuery + callUri.Fragment}";
                    ...
                    {
                        DebugHelper.WriteLine("无效URL :" + request.Url);
                    }
                }
            }
