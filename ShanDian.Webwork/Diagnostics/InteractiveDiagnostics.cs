    ...
    public class InteractiveDiagnostics : IInteractiveDiagnostics
    {
        ...
        public InteractiveDiagnostics(IEnumerable<IDiagnosticsProvider> providers)
        {
            var customProvidersAvailable = providers.Any(provider =>
            {
                ...
                return providerType != typeof(ShanDian.Webwork.Diagnostics.TestingDiagnosticProvider) &
                       providerType != typeof(ShanDian.Webwork.Routing.DefaultRouteCacheProvider);
            });

            if (customProvidersAvailable)
            {
                // Exclude only the TestingDiagnosticProvider
                this.providers = providers.Where(provider => provider.GetType() != typeof(ShanDian.Webwork.Diagnostics.TestingDiagnosticProvider)).ToArray();
            }
            else
            {
