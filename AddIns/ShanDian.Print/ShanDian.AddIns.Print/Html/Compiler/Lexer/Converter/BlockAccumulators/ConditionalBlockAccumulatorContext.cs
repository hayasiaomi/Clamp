...
namespace ShanDian.AddIns.Print.Html.Compiler
{
    internal class ConditionalBlockAccumulatorContext : BlockAccumulatorContext
    {
            ...
            : base(startingNode)
        {
            ...
            {
                throw new HtmlTemplateCompilerException(string.Format(
                        "Tried to convert {0} expression to conditional block", BlockName));
            }
            var testType = BlockName == "if";
            var argument = HtmlTemplateExpression.Boolish(((HelperExpression)startingNode).Arguments.Single());
        ...
        {
            item = UnwrapStatement(item);
            return HtmlTemplateExpression.Boolish(((HelperExpression)item).Arguments.Skip(1).Single());
        }
