    ...
    public class InteractiveDiagnostics : IInteractiveDiagnostics
    {
        ...
        public InteractiveDiagnostics(IEnumerable<IDiagnosticsProvider> providers)
        {
            var customProvidersAvailable = providers.Any(provider =>
            {
                ...
                return providerType != typeof(Clamp.Webwork.Diagnostics.TestingDiagnosticProvider) &
                       providerType != typeof(Clamp.Webwork.Routing.DefaultRouteCacheProvider);
            });

            if (customProvidersAvailable)
            {
                // Exclude only the TestingDiagnosticProvider
                this.providers = providers.Where(provider => provider.GetType() != typeof(Clamp.Webwork.Diagnostics.TestingDiagnosticProvider)).ToArray();
            }
            else
            {
