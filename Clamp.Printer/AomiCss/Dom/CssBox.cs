            ...
            while (startIdx < _text.Length)
            {
                ...
                if (startIdx < _text.Length)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (_text[endIdx + 1] == '#' && CommonUtils.IsDigit(_text[endIdx + 2]))
                            {
                                endIdx = endIdx + 2;

                                while (CommonUtils.IsDigit(_text[endIdx]))
                                    endIdx++;
                            }
                            else
                            {
                                ...
                                while (sIndex < _text.Length && _text[sIndex] != ';' && sIndex < (endIdx + 8))
                                    sIndex++;

                                if (sIndex > (endIdx + 1) && sIndex != (endIdx + 8))
                                    endIdx = sIndex;
                            }
                        }


                        if (endIdx < _text.Length && WordBreak != CssConstants.BreakAll && !CommonUtils.IsAsianCharecter(_text[endIdx]))
                            endIdx++;
