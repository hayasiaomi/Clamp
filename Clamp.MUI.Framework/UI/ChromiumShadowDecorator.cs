...
{
    public class ChromiumShadowDecorator : Form
    {
        private Bitmap _shadowBitmap;
        private Color _shadowColor;
        private int _shadowH = 0;
        private byte _shadowOpacity = 114;
        private int _shadowV = 0;
        private ChromiumWinForm chromiumForm;

        public ChromiumShadowDecorator(ChromiumWinForm chromiumForm)
        {
            this.chromiumForm = chromiumForm;
            this.Owner = chromiumForm;
            this.ShadowColor = Color.FromArgb((int)Math.Round(0.4 * 255), 0, 0, 0);
            this.ShadowBlur = 5;
            this.ShadowSpread = 0;
            ...
            if (this.chromiumForm.Opacity == 0)
            {
                this.Visible = false;
            }

            this.chromiumForm.OnFrmOpacityChanged += (sender, eventArgs) =>
            {
                if (this.chromiumForm != null && this.chromiumForm.Opacity > 0)
                {
                    this.Visible = this.chromiumForm.Visible;
                }
            };

            this.chromiumForm.LocationChanged += UpdateLocation;
            this.chromiumForm.FormClosing += (sender, eventArgs) => this.Close();
            this.chromiumForm.VisibleChanged += (sender, eventArgs) =>
            {
                if (this.chromiumForm != null && this.chromiumForm.Opacity > 0)
                {
                    this.Visible = this.chromiumForm.Visible;
            ...
            this.chromiumForm.SizeChanged += (sender, eventArgs) =>
            {
                if (this.chromiumForm.WindowState != FormWindowState.Normal)
                    this.Visible = false;
                else
                {
                    this.Visible = true;
                    this.chromiumForm.Activate();
                }
            };
            this.chromiumForm.Activated += (sender, args) => this.chromiumForm.BringToFront();
        }
        ...
        public static Bitmap DrawShadowBitmap(int width, int height, int blur, int spread, Color color)
        {
            ...
            // fill background
            g.FillRectangle(new SolidBrush(color), blur, blur, width + spread * 2 + 1, height + spread * 2 + 1);
            // +1 to fill the gap
        ...
        public void UpdateLocation(Object sender = null, EventArgs eventArgs = null)
        {
            ...
            Debug.WriteLine(string.Format("OffsetX:{0},OffsetY:{1}", OffsetX, OffsetY));

            Debug.WriteLine(string.Format("end-x:{0},end-y:{1}", pos.X, pos.Y));
        ...
        public void RefreshShadow(bool redraw = true)
        {
            if (redraw)
            {
                ...
                ShadowBitmap = DrawShadowBitmap(this.Owner.Width, this.Owner.Height, ShadowBlur, ShadowSpread, ShadowColor);
            }
