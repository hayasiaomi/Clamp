...
namespace Clamp.Linker.ViewEngines.Benben
{
    internal static class BuiltinHelpers
    {
        ...
        public static void With(TextWriter output, HelperOptions options, dynamic context, params object[] arguments)
        {
            ...
            {
                throw new BenbenException("{{with}} helper must have exactly one argument");
            }

            if (BenbensUtils.IsTruthyOrNonEmpty(arguments[0]))
            {
        ...
        public static void Inline(TextWriter output, HelperOptions options, dynamic context, params object[] arguments)
        {
            if (arguments.Length != 1)
            {
                throw new BenbenException("{{*inline}} helper must have exactly one argument");
            }
            ...
            {
                throw new BenbenException("{{*inline}} helper must receiving the full BindingContext");
            }
        ...
        public static IEnumerable<KeyValuePair<string, BenbenHelper>> Helpers
        {
            get
            {
                return GetHelpers<BenbenHelper>();
            }
        }

        public static IEnumerable<KeyValuePair<string, BenbenBlockHelper>> BlockHelpers
        {
            get
            {
                return GetHelpers<BenbenBlockHelper>();
            }
        }
