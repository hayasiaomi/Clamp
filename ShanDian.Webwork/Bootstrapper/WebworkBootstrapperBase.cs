namespace ShanDian.Webwork.Bootstrapper
{
    using System;
    ...
    using System.Linq;
    using Diagnostics;
    using ShanDian.Webwork.Cryptography;
    ...
    public abstract class WebworkBootstrapperBase<TContainer> : IWebworkBootstrapper, IControllerCatalog, IDisposable
        where TContainer : class
    {
        ...
        /// </summary>
        private WebworkInternalConfiguration internalConfiguration;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WebworkBootstrapperBase{TContainer}"/> class.
        /// </summary>
        protected WebworkBootstrapperBase()
        {
        ...
        /// </summary>
        protected virtual WebworkInternalConfiguration InternalConfiguration
        {
            get
            {
                return this.internalConfiguration ?? (this.internalConfiguration = WebworkInternalConfiguration.Default);
            }
        }
            ...
            get
            {
                ...
                return
                    ...
                    (this.modules = AppDomainAssemblyTypeScanner
                                        .TypesOf<IController>(ScanMode.ExcludeWebwork)
                                        .NotOfType<DiagnosticController>()
        ...
        protected virtual IEnumerable<Type> TypeConverters
        {
            get
            {
                return AppDomainAssemblyTypeScanner.TypesOf<ITypeConverter>(ScanMode.ExcludeWebwork);
            }
        }
        ...
        {
            get { return AppDomainAssemblyTypeScanner.TypesOf<IBodyDeserializer>(ScanMode.ExcludeWebwork); }
        }
        ...
        public void Initialise()
        {
            ...
            var typeRegistrations = this.InternalConfiguration.GetTypeRegistations().Concat(this.GetAdditionalTypes());

            var collectionTypeRegistrations = this.InternalConfiguration.GetCollectionTypeRegistrations().Concat(this.GetApplicationCollections());
            ...
            var instanceRegistrations = this.Conventions.GetInstanceRegistrations().Concat(this.GetAdditionalInstances());
        ...
        public abstract IEnumerable<IController> GetAllModules(WebworkContext context);
        ...
        public abstract IController GetModule(Type moduleType, WebworkContext context);

        /// <summary>
        /// Gets the configured INancyEngine
        /// </summary>
        /// <returns>Configured INancyEngine</returns>
        public IWebworkEngine GetEngine()
        {
            if (!this.initialised)
            {
            ...
            var engine = this.SafeGetWebworkEngineInstance();
        ...
        /// </summary>
        /// <param name="context">The <see cref="WebworkContext"/> used by the request.</param>
        /// <returns>An <see cref="IPipelines"/> instance.</returns>
        protected virtual IPipelines InitializeRequestPipelines(WebworkContext context)
        {
        ...
        protected virtual void RequestStartup(TContainer container, IPipelines pipelines, WebworkContext context)
        {
        }
        ...
        protected abstract IWebworkEngine GetEngineInternal();
        ...
        private IEnumerable<InstanceRegistration> GetAdditionalInstances()
        {
            return new[] {
                ...
                new InstanceRegistration(typeof(WebworkInternalConfiguration), this.InternalConfiguration),
        ...
        private IWebworkEngine SafeGetWebworkEngineInstance()
        {
            try
            {
        ...
        private static IRootPathProvider GetRootPathProvider()
        {
            var providerTypes = AppDomainAssemblyTypeScanner
                .TypesOf<IRootPathProvider>(ScanMode.ExcludeWebwork)
                .ToArray();
