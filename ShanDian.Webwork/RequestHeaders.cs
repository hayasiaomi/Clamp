namespace ShanDian.Webwork
{
    ...
    using ShanDian.Webwork.Cookies;
    ...
    public class RequestHeaders : IEnumerable<KeyValuePair<string, IEnumerable<string>>>
    {
        ...
        /// </summary>
        /// <value>An <see cref="IEnumerable{T}"/> that contains <see cref="IWebworkCookie"/> instances if they are available; otherwise it will be empty.</value>
        public IEnumerable<IWebworkCookie> Cookie
        {
            get { return this.GetValue("Cookie", GetWebworkCookies); }
        }
        ...
        private static IEnumerable<IWebworkCookie> GetWebworkCookies(IEnumerable<string> cookies)
        {
            if (cookies == null)
            {
                yield break;
            ...
            foreach (var cookie in cookies)
            {
                ...
                foreach (var cookieString in cookieStrings)
                {
                    ...
                    if (equalPos >= 0)
                    {
                        yield return new WebworkCookie(cookieString.Substring(0, equalPos).TrimStart(), cookieString.Substring(equalPos+1).TrimEnd());
                    }
                }
            }
        }
