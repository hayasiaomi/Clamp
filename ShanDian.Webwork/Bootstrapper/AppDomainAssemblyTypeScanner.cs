namespace ShanDian.Webwork.Bootstrapper
{
    ...
    public static class AppDomainAssemblyTypeScanner
    {
        ...
        {
            LoadAssembliesWithWebworkReferences();
        }
        ...
        /// </summary>
        private static bool mWebworkReferencingAssembliesLoaded;

        private static IEnumerable<Func<Assembly, bool>> mAssembliesToScan;
        ...
        {
            x => x == mWebworkAssembly,
            x =>
            {
                return !x.GetName().Name.StartsWith("ShanDian.Webwork.Testing",StringComparison.OrdinalIgnoreCase) &&
                    x.GetReferencedAssemblies().Any(r => r.Name.StartsWith("ShanDian.Webwork", StringComparison.OrdinalIgnoreCase));
            }
        };
        ...
        public static IEnumerable<Func<Assembly, bool>> AssembliesToScan
        {
            private get
            {
                return mAssembliesToScan ?? (mAssembliesToScan = DefaultAssembliesToScan);
            }
            set
            {
                mAssembliesToScan = value;
                UpdateTypes();
            }
        }
        ...
        /// </summary>
        public static void LoadAssembliesWithWebworkReferences()
        {
            if (mWebworkReferencingAssembliesLoaded)
            {
                return;
            }
            ...
            mWebworkReferencingAssembliesLoaded = true;
        }
        ...
        public static IEnumerable<Type> TypesOf(Type type, ScanMode mode)
        {
            ...
            switch (mode)
            {
                case ScanMode.OnlyWebwork:
                    return returnTypes.Where(t => t.Assembly == mWebworkAssembly);
                case ScanMode.ExcludeWebwork:
                    return returnTypes.Where(t => t.Assembly != mWebworkAssembly);
                case ScanMode.OnlyWebworkNamespace:
                    return returnTypes.Where(t => t.Namespace.StartsWith("ShanDian.Webwork"));
                case ScanMode.ExcludeWebworkNamespace:
                    return returnTypes.Where(t => !t.Namespace.StartsWith("ShanDian.Webwork"));
