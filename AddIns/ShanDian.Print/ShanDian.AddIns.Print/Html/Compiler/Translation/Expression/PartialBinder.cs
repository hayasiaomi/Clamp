...
namespace ShanDian.AddIns.Print.Html.Compiler
{
    internal class PartialBinder : HandlebarsExpressionVisitor
    {
...
#else
                new Func<string, BindingContext, HtmlTemplateConfiguration, bool>(InvokePartial).Method,
#endif
        ...
        private static void HandleFailedInvocation(
            string partialName)
        {
            throw new HtmlTemplateRuntimeException(
        ...
        private static bool InvokePartial(
            ...
            HtmlTemplateConfiguration configuration)
        {
            if (configuration.RegisteredTemplates.ContainsKey(partialName) == false)
            {
                if (configuration.FileSystem != null && context.TemplatePath != null)
                {
                    ...
                    if (partialPath != null)
                    {
                        var compiled = HtmlTemplate.Create(configuration)
