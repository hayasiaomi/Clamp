namespace Clamp.Linker.Routing.Trie
{
    ...
    public class RouteResolverTrie : IRouteResolverTrie
    {
        ...
        {
            if (this.routeTries.Count > 0)
            {
                this.routeTries.Clear();
            }
        ...
        public MatchResult[] GetMatches(string method, string path, LinkerContext context)
        {
            ...
            return this.routeTries[method].GetMatches(path.Split(splitSeparators, StringSplitOptions.RemoveEmptyEntries), context).ToArray();
        }
        ...
        public IEnumerable<string> GetOptions(string path, LinkerContext context)
        {
