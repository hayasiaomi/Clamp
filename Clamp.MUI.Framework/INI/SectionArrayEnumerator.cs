namespace Clamp.MUI.Framework.INI
{
    ...
    /// 用于处理数组类型
    /// </summary>
    internal sealed class SectionArrayEnumerator
    {
        ...
        public SectionArrayEnumerator(string value, bool shouldCalcElemString)
        {
            _stringValue = value;
            _idxInString = -1;
            _lastRBraceIdx = -1;
        ...
        public bool Next()
        {
            ...
            while (idx <= _lastRBraceIdx)
            {
                ...
                else if (ch == INIFile.ArrayElementSeparator && _braceBalance == 1 && !_isInQuotes)
                {
                    if (!IsNonEmptyValue(_stringValue, _prevElemIdxInString, idx))
                    {
