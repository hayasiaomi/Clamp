...
namespace ShanDian.AddIns.Print.Html.Compiler
{
    ...
    {
        public static IEnumerable<object> Accumulate(IEnumerable<object> tokens, HtmlTemplateConfiguration configuration)
        {
        ...
        private BlockAccumulator(HtmlTemplateConfiguration configuration)
        {
        ...
        private Expression AccumulateBlock(IEnumerator<object> enumerator, BlockAccumulatorContext context)
        {
            while (enumerator.MoveNext())
            {
                var item = (Expression)enumerator.Current;
            ...
            throw new HtmlTemplateCompilerException($"Reached end of template before block expression '{context.Name}' was closed");
        }
