namespace ShanDian.Webwork
{
    ...
    public class WebworkEngine : IWebworkEngine
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WebworkEngine"/> class.
        /// </summary>
        ...
        public WebworkEngine(IRequestDispatcher dispatcher,
            IWebworkContextFactory contextFactory,
        ...
        public Func<WebworkContext, IPipelines> RequestPipelinesFactory { get; set; }

        public Task<WebworkContext> HandleRequest(Request request, Func<WebworkContext, WebworkContext> preRequest, CancellationToken cancellationToken)
        {
	        var cts = CancellationTokenSource.CreateLinkedTokenSource(this.engineDisposedCts.Token, cancellationToken);
            cts.Token.ThrowIfCancellationRequested();

            var tcs = new TaskCompletionSource<WebworkContext>();
        ...
        private void SaveTraceInformation(WebworkContext ctx)
        {
        ...
        private bool EnableTracing(WebworkContext ctx)
        {
            return StaticConfiguration.EnableRequestTracing &&
                   !ctx.Items.ContainsKey(DiagnosticsHook.ItemsKey);
        }

        private Guid GetDiagnosticsSessionGuid(WebworkContext ctx)
        {
            string sessionId;
        ...
        private void UpdateTraceCookie(WebworkContext ctx, Guid sessionGuid)
        {
        ...
        private void CheckStatusCodeHandler(WebworkContext context)
        {
        ...
        private Task<WebworkContext> InvokeRequestLifeCycle(WebworkContext context, CancellationToken cancellationToken, IPipelines pipelines)
        {
            var tcs = new TaskCompletionSource<WebworkContext>();
        ...
        private Action<Task> PreExecute(WebworkContext context, IPipelines pipelines, TaskCompletionSource<WebworkContext> tcs)
        {
            return postHookTask =>
            {
        ...
        private Action<Task> HandleFaultedTask(WebworkContext context, IPipelines pipelines, TaskCompletionSource<WebworkContext> tcs)
        {
            return t =>
                {
                    try
        ...
        private static Task<Response> InvokePreRequestHook(WebworkContext context, CancellationToken cancellationToken, BeforePipeline pipeline)
        {
            if (pipeline == null)
            {
        ...
        private Task InvokePostRequestHook(WebworkContext context, CancellationToken cancellationToken, AfterPipeline pipeline)
        {
            return pipeline == null ? TaskHelpers.GetCompletedTask() : pipeline.Invoke(context, cancellationToken);
        }

        private void InvokeOnErrorHook(WebworkContext context, ErrorPipeline pipeline, Exception ex)
        {
            try
            {
                if (pipeline == null)
