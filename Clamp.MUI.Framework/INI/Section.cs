...
namespace Clamp.MUI.Framework.INI
{
    ...
    /// 节项类
    /// </summary>
    public sealed class Section : INIElement
    {
        ...
        public Section(string name) : this(name, string.Empty)
        { }


        public Section(string name, object value) : base(name)
        {
            this.SetValue(value);
            this._cachedArrayElementSeparator = INIFile.ArrayElementSeparator;
        }
        ...
        public int ArraySize
        {
            get
            {
                if (this._cachedArrayElementSeparator != INIFile.ArrayElementSeparator)
                {
                    this._cachedArrayElementSeparator = INIFile.ArrayElementSeparator;
        ...
        {
            int size = 0;
            var enumerator = new SectionArrayEnumerator(_rawValue, false);
        ...
        public object GetValue(Type type)
        {
            ...
            if (type.IsArray)
                throw new InvalidOperationException(StringResources.INI_SectionItem_TypeShouldArray);

            if (this.IsArray)
                throw new InvalidOperationException(StringResources.INI_SectionItem_ShouldArray);
        ...
        public object[] GetValueArray(Type elementType)
        {
            ...
            if (myArraySize > 0)
            {
                var enumerator = new SectionArrayEnumerator(_rawValue, true);
                int iElem = 0;
                while (enumerator.Next())
                {
        ...
        private static object CreateObjectFromString(string value, Type dstType)
        {
            ...
            var converter = INIFile.FindTypeStringConverter(dstType);
            ...
            {
                throw SectionValueCastException.Create(value, dstType, ex);
            }
        }
        ...
        public void SetValue(object value)
        {
            ...
            if (type.IsArray)
            {
                ...
                for (int i = 0; i < values.Length; i++)
                {
                    ...
                    var converter = INIFile.FindTypeStringConverter(elemValue.GetType());
                    strings[i] = converter.ConvertToString(elemValue);
                }

                _rawValue = string.Format("{{{0}}}", string.Join(INIFile.ArrayElementSeparator.ToString(), strings));
                _cachedArraySize = values.Length;
                _shouldCalculateArraySize = false;
            }
            else
            {
                var converter = INIFile.FindTypeStringConverter(type);
        ...
        private static ArgumentException CreateJaggedArraysNotSupportedEx(Type type)
        {
            ...
            throw new ArgumentException(string.Format(StringResources.INI_SectionItem_JaggedArrays, type.Name, elementType.Name));
        }
    }
}
