            ...
            foreach (CssBox b in box.Boxes)
            {
                ...
                if (b.Words.Count > 0)
                {
                    ...
                    foreach (var word in b.Words)
                    {
                        ...
                        //DebugHelper.WriteLine("{0}(wrapNoWrapBox:{1},word.IsLineBreak :{2},b.WhiteSpace:{3})", word.Text.Replace(' ', '-').Replace("\n", "\\n"), wrapNoWrapBox, word.IsLineBreak, b.WhiteSpace);

                        //DebugHelper.WriteLine("{0}(curx:{1},word.Width:{2}rightspacing:{3},sum:{4},limitRight: {4})", word.Text.Replace(' ', '-').Replace("\n", "\\n"), curx, word.Width, rightspacing, curx + word.Width + rightspacing, limitRight);
                            ...
                            || word.IsLineBreak || wrapNoWrapBox)
                        {
                            ...
                            //DebugHelper.WriteLine("{0}(maxbottom:{1},linespacing:{2} sum: {3})", word.Text.Replace(' ', '-').Replace("\n", "\\n"), maxbottom, linespacing, maxbottom + linespacing);
                        ...
                        //DebugHelper.WriteLine("{0}(x:{1},y:{2})", word.Text.Replace(' ', '-').Replace("\n", "\\n"), curx, cury);
