namespace ShanDian.Webwork
{
    ...
    public class Request : IDisposable
    {
        ...
        private void ParseFormData()
        {
            ...
            else if (Json.Json.IsJsonContentType(mimeType))
            {
                var reader = new StreamReader(this.Body);
                ...
                {
                    Dictionary<string, object> keyValues = JsonConvert.DeserializeObject<Dictionary<string, object>>(dataJson);

                    string data = string.Join("&", keyValues.Select(kv => $"{kv.Key}={kv.Value}").ToArray());

                    this.form = data.AsQueryDictionary();
                    this.Body.Position = 0;
                }
            }
