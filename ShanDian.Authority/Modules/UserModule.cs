...
{
    public class UserModule : WebworkModule
    {
        private readonly IUserService userServices;

        public UserModule() : base("Account")
        {
        ...
        {
            SDResponse<LoginDto> response = new SDResponse<LoginDto>();
        ...
        {
            SDResponse<List<UserSummaryDto>> response = new SDResponse<List<UserSummaryDto>>();
        ...
        {
            SDResponse<UserDetailDto> response = new SDResponse<UserDetailDto>();
        ...
        [Post("users")]
        public dynamic AddUser(UserInDto userInDto)
        {
        ...
        [Put("users/userId")]
        public dynamic AlterUser(UserInfoInDto userInfoInDto)
        {
            SDResponse<object> response = new SDResponse<object>();
        ...
        {
            SDResponse<List<SimpleUserDto>> response = new SDResponse<List<SimpleUserDto>>();
            ...
            if (apiResponse.IsSuccess())
            {
                if (apiResponse.Data != null)
                {
                    response.Result = apiResponse.Data.Where(g => g.IsDefaultAdmin == false).Select(t => (new SimpleUserDto { UserId = t.Id, UserName = t.NickName })).ToList();
                    response.Flag = true;
                    response.Code = 200;
                    response.Message = apiResponse.Msg;
                }
        ...
        public dynamic AlterUserRole(UserRolePermissionInDto userRolePermissionInDto)
        {
            SDResponse<object> response = new SDResponse<object>();
        ...
        public dynamic VerificationUser(VerificationUserInDto verificationUserInDto)
        {
            SDResponse<object> response = new SDResponse<object>();
