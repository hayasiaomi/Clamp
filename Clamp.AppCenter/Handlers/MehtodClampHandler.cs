...
{
    public abstract class MehtodClampHandler : DefaultClampHandler
    {
        ...
        private Dictionary<string, MethodInfo> methodCaches = new Dictionary<string, MethodInfo>();
        ...
        {
            if (this.methodCaches.ContainsKey(methodName))
                return this.methodCaches[methodName];

            MethodInfo methodInfo = this.GetType().GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                .FirstOrDefault(mi => string.Equals(mi.Name, methodName, StringComparison.CurrentCultureIgnoreCase));

            if (methodInfo != null)
            {
                this.methodCaches.Add(methodName, methodInfo);
            }
        ...
        private object[] GetMethodParameterInfos(CfxRequest cfxRequest, MethodInfo methodInfo)
        {
            ...
            object[] mParams = new object[parameterInfos.Length];
        ...
        private Dictionary<string, string> GetHandleParameters(CfxRequest cfxRequest)
        {
            if (string.IsNullOrWhiteSpace(cfxRequest.Url))
            {
        ...
        private NameValueCollection ParseQueryString(string query, Encoding encoding, bool caseSensitive)
        {
            if (query == null)
        ...
        private KeyValuePair<string, string> ParseQueryStringSegment(string segment, Encoding encoding)
        {
        ...
        private string UrlDecode(byte[] bytes, Encoding e)
        {
            if (bytes == null)
                return null;
        ...
        private string UrlDecode(byte[] bytes, int offset, int count, Encoding e)
        {
            if (bytes == null)
                return null;
            if (count == 0)
        ...
        private void WriteCharBytes(System.Collections.IList buf, char ch, Encoding e)
        {
            if (ch > 255)
            {
