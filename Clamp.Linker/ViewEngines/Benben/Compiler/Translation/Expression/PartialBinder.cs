...
#if netstandard
                new Action<string, BindingContext, BenbenConfiguration>(InvokePartialWithFallback).GetMethodInfo(),
#else
                new Action<string, BindingContext, BenbenConfiguration>(InvokePartialWithFallback).Method,
#endif
            ...
            BenbenConfiguration configuration)
        {
            if (!InvokePartial(partialName, context, configuration))
            {
                if (context.PartialBlockTemplate == null)
                {
                    ...
                    {
                        throw new BenbenRuntimeException(string.Format("Referenced partial name {0} could not be resolved", partialName));
                    }
                }
            ...
            BenbenConfiguration configuration)
        {
            ...
            if (configuration.RegisteredTemplates.ContainsKey(partialName) == false)
            {
                if (configuration.PartialTemplateResolver == null 
                    || configuration.PartialTemplateResolver.TryRegisterPartial(Benben.Create(configuration), partialName, context.TemplatePath) == false)
                {
                    // Template not found.
                    return false;
                }
            ...
            {
                throw new BenbenRuntimeException(
